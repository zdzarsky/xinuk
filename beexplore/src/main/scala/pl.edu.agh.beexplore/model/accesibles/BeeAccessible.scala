...
object BeeAccessible {
  ...
  def unapply(arg: EmptyCell)(implicit config: BeexploreConfig): BeeAccessible[Bee] = {
    (smell: Signal, id: Int, numberOfFlights: Int, experience: Experience, hunger: Int) => Bee(id, arg.smellWith(smell), numberOfFlights, experience, hunger)
  }

  def unapply(arg: BufferCell)(implicit config: BeexploreConfig): BeeAccessible[BufferCell] =
    (smell: Signal, id: Int, numberOfFlights: Int, experience: Experience, hunger: Int) => BufferCell(Bee(id, arg.smellWith(smell), numberOfFlights, experience, hunger))
}
